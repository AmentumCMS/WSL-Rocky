...
Automated build pipeline for a hardened, STIG-compliant Rocky Linux 9 WSL2 image
...
|----------|-------------|
| `wsl-rocky-<ver>-<build>.tar.gz` | WSL2-importable Rocky Linux rootfs |
...
[GitHub Container Registry (GHCR)](../../pkgs/container/wsl-rocky).
...
$tag    = (Invoke-RestMethod https://api.github.com/repos/<owner>/WSL-Rocky/releases/latest).tag_name
$asset  = "wsl-rocky-9-${tag}.tar.gz"
Invoke-WebRequest "https://github.com/<owner>/WSL-Rocky/releases/download/${tag}/${asset}" -OutFile $asset

# 2. Verify the checksum (compare against SHA256SUMS from the same release)
Get-FileHash $asset -Algorithm SHA256

# 3. Import into WSL2
wsl --import Rocky-Hardened C:\WSL\Rocky-Hardened .\$asset --version 2

# 4. Launch
wsl -d Rocky-Hardened
```
...
docker pull ghcr.io/<owner>/wsl-rocky:latest

# Export as a WSL tarball (on Windows with Docker Desktop)
docker export $(docker create ghcr.io/<owner>/wsl-rocky:latest) -o wsl-rocky.tar
# Then: wsl --import ...
```
...
├── packages.list          ← Extra dnf packages to install
...
│                                                                     │
│  1. docker pull rockylinux:<version>                                │
...
│  4. docker commit → wsl-rocky:local  (for GHCR push)               │
...
│  10. docker push → ghcr.io/<owner>/wsl-rocky                       │
...
| Manual dispatch | Choose Rocky Linux version, SCAP profile |
...
The pipeline uses the RHEL 9 SCAP content (`ssg-rhel9-ds.xml`), which is
applicable to Rocky Linux 9 as a binary-compatible RHEL derivative.
...
3. **MSIX/Store distribution** — distributing via the Microsoft Store requires
